...
def run(rank, n_gpus, hps):
    ...
    if (
        ...
        print(f"Using transformer flows {transformer_flow_type} for VITS3")
        assert (
    ...
    if (
        ...
        print("Using noise scaled MAS for VITS3")
    ...
    if (
        ...
        # print("Using duration discriminator for VITS3")
        ...
        print(f"Using duration_discriminator {duration_discriminator_type} for VITS3")
        assert (
    ...
    for batch_idx, (
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            with autocast(enabled=False):
                ...
                loss_kl = kl_loss(x_flown, logs_q, m_p, logs_p, z_mask) * hps.train.c_kl
                prior_loss = torch.sum(0.5 * ((z.detach() - x_expanded) ** 2 + math.log(2 * math.pi)) * y_mask)
                prior_loss = prior_loss / (torch.sum(z_mask) * net_g.spec_channels)
